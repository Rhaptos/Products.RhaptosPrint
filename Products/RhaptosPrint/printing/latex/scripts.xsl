...
-->
		...
		<xsl:when test="$over='&#x00302;'"><!-- caret - hat -->
			<xsl:choose>
			  <xsl:when test="string-length($base) &gt; 1">
		...
		<xsl:when test="$over='&#x007E;' or $over='&#x02DC;'"><!-- tilde  -->
			<xsl:text>\tilde{</xsl:text>
			<xsl:apply-templates select="./*[1]"/>
			<xsl:text>}</xsl:text>
		</xsl:when>
		<xsl:when test="$over='&#x00A8;' or $over='&#x0168;' or $over='&#x0308;' or $over='&#x0776;'"><!-- diaresis  -->
			<xsl:text>\ddot{</xsl:text>
			<xsl:apply-templates select="./*[1]"/>
			<xsl:text>}</xsl:text>
		</xsl:when>
		<xsl:when test="$over='&#x02D9;' or $over='&#x0729;' "><!-- single dot  -->
