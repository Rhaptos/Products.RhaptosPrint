...
<!-- ====================================================================== -->
<!-- $id: entities.xsl, 2002/17/05 Exp $
...
<!-- ====================================================================== -->
		...
		<xsl:when test="starts-with($content,'&#x0211D;')"><xsl:value-of select="'\mathbb{R} '" /><xsl:call-template name="replaceEntities"><xsl:with-param name="content" select="substring-after($content, '&#x0211D;')"/></xsl:call-template></xsl:when>	<!--/mathbb{R} - set of reals --> <!-- Requires amsfonts or amssymb -->
...
<!-- ====================================================================== -->		
		...
		<xsl:when test='starts-with($content,"[")'><xsl:value-of select='"\left["' /><xsl:call-template name="replaceEntities"><xsl:with-param name="content" select="substring-after($content, '[')"/></xsl:call-template></xsl:when>
		<xsl:when test='starts-with($content,"]")'><xsl:value-of select='"\right]"' /><xsl:call-template name="replaceEntities"><xsl:with-param name="content" select="substring-after($content,
']')"/></xsl:call-template></xsl:when>
		...
		<xsl:when test='starts-with($content,"\lessthan{}")'>
			<xsl:value-of select='"\lessthan{}"' />
			<xsl:call-template name="replaceEntities">
				<xsl:with-param name="content" select="substring-after($content,'}')"/>
			</xsl:call-template>
		</xsl:when>
	<xsl:when test='starts-with($content,"\greatthan{}")'>
			<xsl:value-of select='"\greatthan{}"' />
			<xsl:call-template name="replaceEntities">
				<xsl:with-param name="content" select="substring-after($content,'}')"/>
			</xsl:call-template>
		</xsl:when>
	  <xsl:when test='starts-with($content,"\ensuremath{\ll}")'>
			<xsl:value-of select='"\ensuremath{\ll}"' />
			<xsl:call-template name="replaceEntities">
				<xsl:with-param name="content" select="substring-after($content,'}')"/>
			</xsl:call-template>
		</xsl:when> 
    <xsl:when test='starts-with($content,"\ensuremath{\gg}")'>
			<xsl:value-of select='"\ensuremath{\gg}"' />
			<xsl:call-template name="replaceEntities">
				<xsl:with-param name="content" select="substring-after($content,'}')"/>
			</xsl:call-template>
		</xsl:when>
		<xsl:when test='starts-with($content,"\ensuremath{\sim}")'>
			<xsl:value-of select='"\ensuremath{\sim}"' />
			<xsl:call-template name="replaceEntities">
				<xsl:with-param name="content" select="substring-after($content,'}')"/>
			</xsl:call-template>
		</xsl:when>  
...
<xsl:template name="replaceMtextEntities">
  ...
  <xsl:when test='contains($content,"\#")'>
	  <xsl:call-template name="replaceMtextEntities"><xsl:with-param name="content" select="substring-before($content,'\#')"/></xsl:call-template>
	  <xsl:value-of select="'\#'"/>
	  <xsl:call-template name="replaceMtextEntities"><xsl:with-param name="content" select="substring-after($content,'\#')"/></xsl:call-template>
  </xsl:when>
	<xsl:when test='contains($content,"#")'>
	  <xsl:call-template name="replaceMtextEntities"><xsl:with-param name="content" select="substring-before($content,'#')"/></xsl:call-template>
	  <xsl:value-of select="'\#'"/>
  ...
  <xsl:when test="contains($content,'~')">
		<xsl:call-template name="replaceMtextEntities">
			<xsl:with-param name="content" select="concat(substring-before($content,'~'),'\textasciitilde{}',substring-after($content,'~'))"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:when test="contains($content,'\ensuremath{\sim}')">
		<xsl:call-template name="replaceMtextEntities">
			<xsl:with-param name="content" select="concat(substring-before($content,'\ensuremath{\sim}'),'\textasciitilde{}',substring-after($content,'\ensuremath{\sim}'))"/>
		</xsl:call-template>
	</xsl:when>
	...
	<xsl:when test="starts-with($content,' ')"> <!-- Preserve leading space -->
		<xsl:call-template name="replaceMtextEntities">
			<xsl:with-param name="content" select="concat('\hspace{0.17em}', substring-after($content,' '))"/>
		</xsl:call-template>
	</xsl:when>
  <xsl:when test="substring($content,string-length($content)) = ' '"> <!-- Preserve trailing space -->
		<xsl:call-template name="replaceMtextEntities">
			<xsl:with-param name="content" select="concat(substring($content, 1, string-length($content)-1), '\hspace{0.17em}')"/>
		</xsl:call-template>
	</xsl:when>
	<xsl:otherwise>
