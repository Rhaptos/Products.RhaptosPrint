...
<xsl:param name="cnx.font.catchall">STIXGeneral,STIXSizeOneSym,Code2000</xsl:param>
...
<xsl:template match="db:token[@class='simplemath']/text()">
    <xsl:choose>
        <xsl:when test="normalize-space(.) != '' and normalize-space(.) != ' '">
            <fo:inline font-family="STIXGeneral,STIXSizeOneSym">
                <xsl:value-of select="."/>
            </fo:inline>
        </xsl:when>
        <xsl:otherwise>
  ...
  <fo:inline font-weight="bold">
    <xsl:if test="ancestor::db:token[@class='simplemath']">
        <xsl:attribute name="font-family">
            <xsl:text>STIXGeneral,STIXSizeOneSym</xsl:text>
        </xsl:attribute>
    </xsl:if>
    <xsl:if test="@dir">
